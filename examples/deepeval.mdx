...
1. **DeepEvalEvaluatorConfig** (`config.py`)
...
2. **DeepEvalEvaluator** (`evaluator.py`)
...
4. **Setup Script** (`setup_controls.py`)
...
5. **Entry Point Registration** (`pyproject.toml`)
...
control_definition = {
    ...
    "definition": {
        ...
        "condition": {
            "selector": {"path": "*"},  # Pass full data (input + output)
            "evaluator": {
...
- `condition.selector.path: "*"` - Pass full data so evaluator gets both input and output
...
```bash
# In separate shell, Navigate to the DeepEval example directory
...
This example shows the **evaluator architecture** for extending agent-control. While this specific example is set up for monorepo development, the same pattern works for third-party evaluators using published packages.
...
- **DeepEval Documentation**: [https://deepeval.com/docs/metrics-llm-evals](https://deepeval.com/docs/metrics-llm-evals)
{/*vale Vale.Terms = YES*/}
- **G-Eval Guide**: [https://www.confident-ai.com/blog/g-eval-the-definitive-guide](https://www.confident-ai.com/blog/g-eval-the-definitive-guide)
- **Agent Control Evaluators**: [Base evaluator class](https://github.com/agentcontrol/agent-control/blob/main/evaluators/builtin/src/agent_control_evaluators/_base.py)
- **CrewAI Example**: [Using agent-control as a consumer](/examples/crewai)
...
[`DeepEval Custom Evaluator Example`](https://github.com/agentcontrol/agent-control/tree/main/examples/deepeval)

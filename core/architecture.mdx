...
flowchart TD
    ...
    subgraph Ecosystem["Evaluator Ecosystem"]
        direction LR
        Regex["Regex\nEvaluator"] ~~~ List["List\nEvaluator"] ~~~ Luna["Luna-2\nEvaluator"] ~~~ Custom["Custom\nEvaluators"]
    end
```
...
- [Server](/components/server) — FastAPI control-plane API, auth, and persistence.
- [Engine](/components/engine) — Evaluation core that runs controls and orchestrates evaluators.
- [Models](/components/models) — Shared Pydantic models for API and SDK contracts.
- [Evaluators](/components/evaluators) — Built-in and extensible evaluators for detection logic.
- [UI Dashboard](/components/ui) — Web dashboard for managing agents and controls.

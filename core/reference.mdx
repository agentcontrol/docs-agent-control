...
| Method | Endpoint | Description |
|--------|----------|-------------|
| `PUT` | `/controls` | Create control (accepts raw or [template](/concepts/control-templates) input) |
| `GET` | `/controls` | List controls (supports `template_backed` filter) |
| `GET` | `/controls/{control_id}` | Get control |
| `GET` | `/controls/{control_id}/data` | Get control data (includes template metadata if template-backed) |
| `PUT` | `/controls/{control_id}/data` | Update control data (re-renders for template-backed controls) |
| `PATCH` | `/controls/{control_id}` | Update control metadata (`name`, `enabled`) |
...
| Method | Endpoint | Description |
|--------|----------|-------------|
| `POST` | `/control-templates/render` | Preview a rendered control from a template without persisting |

See [Control Templates](/concepts/control-templates) for the full template authoring guide.

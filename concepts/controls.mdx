---
title: Controls
description: Understand how controls work and how to define them.
---

![Control Components](./Controls.png)
...
Controls can also be created from **parameterized templates**. Instead of writing the full JSON definition, you define a template with typed parameters (`string`, `string_list`, `enum`, `boolean`, `regex_re2`) and provide values — the server renders the concrete control for you.

Template-backed controls are evaluated identically to raw controls. Templates are an authoring convenience with zero runtime overhead.

See [Control Templates](/concepts/control-templates) for the full guide, or [How-to: Create a control from a template](/how-to/create-control-from-template) for a step-by-step walkthrough.

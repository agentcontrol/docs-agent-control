...
The SDK provides helpers for working with [control templates](/concepts/control-templates):
...
async with AgentControlClient() as client:
    rendered = await controls.render_control_template(
        client,
        template={
        ...
        template_values={"pii_pattern": r"\b\d{3}-\d{2}-\d{4}\b"},
    )
```

### Create a Control from a Template

Pass `template` and `template_values` in the `data` argument of `create_control()`:
...
### Convert a Stored Control Back to Template Input

```python
template_input = controls.to_template_control_input(control_data)
```

### List Template-Backed Controls

```python
template_controls = await controls.list_controls(client, template_backed=True)
```

---
title: Observability
description: Real-time monitoring and analytics for control executions — stats, events, and API endpoints.
icon: "chart-line"
---
...
flowchart TD
    subgraph sources["EVENT SOURCES"]
        SDK["SDK<br/>(local controls)"]
        SRV["Server<br/>(remote controls)"]
    end

    SDK -- "HTTP POST" --> merge((" "))
    SRV -- "Direct call" --> merge

    merge --> ingestor

    subgraph ingestor["EventIngestor (Protocol)"]
        direction LR
        subgraph bi1["Built-in"]
            DEI["DirectEventIngestor<br/>(sync processing)"]
        end
        subgraph up1["User-provided"]
            QEI["QueuedEventIngestor<br/>(asyncio.Queue)"]
            KEI["KafkaEventIngestor"]
            RMI["RabbitMQIngestor"]
            REI["RedisEventIngestor"]
        end
    end

    ingestor --> store

    subgraph store["EventStore (ABC)"]
        direction LR
        subgraph bi2["Built-in"]
            PES["PostgresEventStore<br/>(JSONB + indexes)"]
        end
        subgraph up2["User-provided"]
            CES["ClickhouseEventStore<br/>(columnar, fast)"]
            TDS["TimescaleDBStore<br/>(time-series)"]
        end
    end
```

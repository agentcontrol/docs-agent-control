...
This section centralizes the technical reference for Agent Control. For concepts and architecture, head to the [Concepts](/concepts) section.

## SDK Reference

Use the SDK docs for language-specific details, examples, and API surface area.
...
When API key auth is enabled, include the key in the `X-API-Key` header. For configuration details and environment variables, see [Configuration](/configuration).
...
- [Testing](/testing)
- [Configuration](/configuration)# Agent Control Reference
